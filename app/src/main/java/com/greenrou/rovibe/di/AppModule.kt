...
val appModule = module {
    ...
    single<SoundEngine> { AudioTrackSoundEngine(get()) }
    single { SoundCommandRepository(get()) }
    single { CompositionStore(androidContext()) }
    single { CompositionRepository(get()) }
    viewModel { HomeViewModel(get(), get()) }
    viewModel { (itemId: String?) -> CreateViewModel(get(), get(), get(), itemId) }
    viewModel { CompositionsViewModel(get(), get(), get()) }
    viewModel { (compositionId: String?) -> CompositionEditorViewModel(get(), get(), get(), compositionId) }
}
